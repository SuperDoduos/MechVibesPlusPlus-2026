...
const APP_TITLE = 'MechVibes++ 2026';
...
function sendInputEvent(channel, payload = {}) {
...
function registerInputHook() {
  ...
  isInputHookRegistered = true;
}

function startInputHook() {
  if (isInputHookStarted) {
    return;
  }
  uIOhook.start();
  isInputHookStarted = true;
}
  ...
  app.on('ready', () => {
    ...
    const contextMenu = Menu.buildFromTemplate([
      ...
      },
      {
        ...
        click: function () {
          shell.openPath(keyboardcustom_dir);
        },
      },
      {
        label: 'Mouse Sound Custom Folder',
        click: function () {
          shell.openPath(mousecustom_dir);
        },
      },
      {
        label: 'Open Devtools',
